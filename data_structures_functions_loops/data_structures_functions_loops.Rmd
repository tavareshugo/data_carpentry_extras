...
A common example of the use of arrays is to store image data (e.g. red, blue and green intensity channels).
...
We should not get a warning when there are infinite values or an error if the vector is not character:
...
When adding results to an object with a for loop, **always create an empty object first**. 
In the example above, we created an empty list of length 5. 
...
The answer to this exercise is in a [separate document](data_structures_functions_loops/gapminder_functions_iteration_solutions.html)
