...
_Note:_ This lesson has been used as an extra in some 
[Data Carpentry in R](http://www.datacarpentry.org/R-ecology-lesson/) 
courses, and thus assumes that some of those lessons were covered beforehand. 
...
norm_cts %>% 
  ...
  full_join(test_result, by = "gene") %>% 
  filter(padj < 0.05) %>% 
  mutate(sample = reorder(sample, rank(dex)),
...
This is because the `reorder()` function works best if the variable we're using 
to define the order of the levels is numeric. In this case, we could do the following:
...
### THIS CODE CHUNK IS DEPRECATED - USE reorder() INSTEAD
animal <- c("mouse", "elephant", "cat", "mouse", "elephant")
